...
def checkBug_changed(OB):
    if OB.get() == True:
        file = open('options/Bug.txt', 'w')
        file.write('True')
        file.close()
    elif OB.get() == False:
        file = open('options/Bug.txt', 'w')
...
if Lang == "RU":
    ...
    chk2 = Checkbutton(window, text='Баг?', variable=stateOB, command=lambda: checkBug_changed(stateOB))
    canvas.create_window((20, 120), anchor="nw", window=chk2, width=60, height=30)
    ...
    chk2 = Checkbutton(window, text='Bug?', variable=stateOB, command=lambda: checkBug_changed(stateOB))
    canvas.create_window((20, 120), anchor="nw", window=chk2, width=60, height=30)
    ...
    chk2 = Checkbutton(window, text='虫子?', variable=stateOB, command=lambda: checkBug_changed(stateOB))
    canvas.create_window((20, 120), anchor="nw", window=chk2, width=60, height=30)
